...
def on_msg_offset(agent, *offset: tuple):
    ...
    if float(offset[0]) > 0 and offset[1] in ["L", "R"]:
        ...
        if dirto_status == False:
            ...
            print("\n%s%s OFFSET AFTER DIRTO" %(offset[0],offset_side))
    else:
...
def envoi_vent():
    IvySendMsg("WindComponent WindVelocity=%s WindDirection=%s" %(fp.wind[V_VENT], fp.wind[DIR_VENT])) #envoi pour les autres modules
    IvySendMsg("WindComponent VWind=%s dirWind=%s" %(fp.wind[V_VENT], fp.wind[DIR_VENT])) # abonnement du modèle de vol :^WindComponent\sVWind=(\S+)\s+dirWind=(\S+)
...
def envoi_perfo():
    perfos = perf.perfos_avion(volets, landing_gear, float(StateVector[ALT_SV])) #récupération des perfos en fonction de la config avion et de l'altitude
...
#en entrée : N/A
#en sortie : envoi de la vitesse/mach managée sur le bus IVY en fonction de l'alitude de l'avion
def envoi_vitesse():
    alt = c.meters_to_feet(float(StateVector[ALT_SV]))
    ###IAS ou altitude sous la trans alt (limité 250kn en dessous de 10000ft)
    ...
    if alt > TRANS_ALT_FT:
        ...
        return
    IvySendMsg("Statut=Vc MachManaged=0 VcManaged=%s" %(c.knots_to_ms(fp.cost_index_ias()))) #envoi de la vitesse managée par défaut si erreur dans les tests d'altitude
...
def envoi_altitude(flight_plan :list):
    if wpt_courant == len(flight_plan)-1: #envoie de l'altitude du dernier WPT si on est sur le dernier WPT
        altitude = flight_plan[wpt_courant][ALT_FP]
    else:
        altitude = flight_plan[wpt_courant+1][ALT_FP] #envoie de l'altitude du prochain WPT
    if altitude == fp.UNDEFINED_Z and float(StateVector[ALT_SV]) >= 0:  #si l'altitude est indéfinie, on envoie l'altitude actuelle
        altitude = StateVector[ALT_SV]
    else:
        altitude = 1000 #altitude par défaut
...
def sequencement_leg(flight_plan :list[list]):
    ...
    if dirto_status == True:
        ...
        if flight_plan[wpt_courant][TYPE_WPT_FP] == OVERFLY:
            if distance_to_go(xy_avion, xy_wpt_dirto) < FACTEUR*epsilon_overfly:
                dirto_status = False
                print("\n\nDIRTO %s FINISHED", (flight_plan[wpt_courant][NOMWPT_FP]))
                print("Sequencement %s" %(flight_plan[wpt_courant][NOMWPT_FP]))
                return wpt_courant
    ...
    if flight_plan[wpt_courant+1][TYPE_WPT_FP] == FLYBY:
        if distance < distance_to_end_leg(xy_previous_wpt, xy_wpt, xy_wpt_suivant):
            print("\n\nSequencement %s" %(flight_plan[wpt_courant+1][NOMWPT_FP]))
...
def distance_to_end_leg(xy_waypoint1, xy_waypoint2, xy_waypoint3):
    ...
    if distance_virage < epsilon_overfly: #si la distance de virage est inférieure à l'epsilon overfly, on prend l'epsilon overfly (ex: deux waypoints alignées alors d=0)
...
def xy_offset(x_wpt, y_wpt): #pas certains que les coordonnées calculées soient correctes
    ...
    if offset_side == "R":
        ...
        y = y_wpt + offset_dist*math.sin(90-cap)
        return [x, y]
    return -1 #erreur dans le format de l'offset (pas pris en compte dans la suite du programme)
