...
def plt_flightplan(flightplan):
    ...
    for i in range(len(fp)):
        if i + 1 < len(fp):
            ...
            plt.plot((fp[i].x, fp[0].x), (fp[i].y, fp[0].y), marker="D", color="red")
...
def vertical(position_avion):
    ...
    IvySendMsg("InitStateVector x=%s y=%s z=0 Vp=%s fpa=0.0000 psi=0 phi=0.0000" %
               (str(position_avion.x), str(position_avion.y), str(c.knots_to_ms(c.ias_to_tas(100, 0)))))
...
def on_msg_ZcManaged(agent, *data):
    global ZcM
    ZcM = float(data[0])
    plt.annotate(f"Z Consigne : {ZcM}", (1000, 1000))
